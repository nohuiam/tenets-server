...
export function createRecordEvaluationHandler(db) {
    return async (input) => {
        ...
        if (input.violations && input.violations.length > 0) {
            for (const violation of input.violations) {
                ...
                const existingPattern = existingPatterns.find(p => violation.toLowerCase().includes(p.description.toLowerCase()) ||
        ...
        const assessmentPattern = existingPatterns.find(p => p.description === assessmentPatternDesc && p.pattern_type === 'success');
