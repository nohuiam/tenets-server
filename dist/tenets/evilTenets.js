 ...
 */
export function getEvilTenet(id) {
    return EVIL_TENETS[id];
}
/**
 * Get evil tenet by its opposite Christ tenet name
 */
export function getEvilTenetByOpposite(christTenetName) {
    const upperName = christTenetName.toUpperCase();
    return Object.values(EVIL_TENETS).find(tenet => tenet.opposite_of.toUpperCase() === upperName);
}
/**
 * Get the tenet pair (Christ + Evil) by ID
 */
export function getTenetPair(id) {
 ...
 */
export const TENET_PAIRS = Object.values(EVIL_TENETS).map(t => ({
    id: t.id,
