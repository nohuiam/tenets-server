...
export function createHttpServer(db, evaluator, counterfeitDetector, port) {
    ...
    app.get('/api/tenets/category/:category', (req, res) => {
        try {
            // SECURITY: Validate category parameter
            const validCategories = ['foundation', 'action', 'character', 'community', 'restoration'];
    ...
    app.post('/api/check-counterfeit', (req, res) => {
        try {
            // SECURITY: Validate input with Zod schema
    ...
    app.post('/api/blind-spots', (req, res) => {
        try {
            // SECURITY: Validate input with Zod schema
    ...
    app.get('/api/evaluations', (req, res) => {
        try {
            ...
            const validAssessments = ['affirm', 'caution', 'reject'];
            const assessmentParam = req.query.assessment;
            const assessment = assessmentParam && validAssessments.includes(assessmentParam)
                ? assessmentParam
                : undefined;
    ...
    app.get('/api/violations', (req, res) => {
        try {
            ...
            const tenetId = req.query.tenet_id ? parseInt(req.query.tenet_id, 10) : undefined;
