...
export class CounterfeitDetector {
  ...
  check(
    ...
    const matchedCounterfeits: CounterfeitMatch[] = [];
    ...
    const authenticCount = CounterfeitDetector.AUTHENTIC_INDICATORS.filter(
      ind => textLower.includes(ind)
    ).length;
    ...
    let authenticAlternative: string | undefined;
   ...
   */
  private checkTenetCounterfeits(tenet: Tenet, textLower: string): CounterfeitMatch[] {
    const matches: CounterfeitMatch[] = [];
  ...
  private checkUniversalPatterns(
    ...
    for (const pattern of UNIVERSAL_COUNTERFEIT_PATTERNS) {
      const matchedIndicators = pattern.indicators.filter(ind =>
        textLower.includes(ind.toLowerCase())
      );
   ...
   */
  private extractSignificantWords(phrase: string): string[] {
   ...
   */
  private deduplicateMatches(matches: CounterfeitMatch[]): CounterfeitMatch[] {
    const seen = new Map<string, CounterfeitMatch>();
  ...
  private generateExplanation(
    ...
    parts.push(
      `Primary concern: "${primary.counterfeit_pattern}" (${Math.round(primary.confidence * 100)}% confidence)`
    );
