...
export class Evaluator {
  ...
  evaluate(
    ...
    // Score each tenet
    const tenetScores: Record<number, number> = {};
    const violations: Violation[] = [];
    const counterfeitMatches: CounterfeitMatch[] = [];
  ...
  private scoreTenet(
    ...
    score: number;
    violation?: Violation;
    counterfeitMatch?: CounterfeitMatch;
  } {
    let score = 0.5; // Start neutral
    let violation: Violation | undefined;
    let counterfeitMatch: CounterfeitMatch | undefined;
   ...
   */
  private runPrimaryTests(textLower: string): Record<string, boolean> {
  ...
  private calculateAssessment(
    ...
    counterfeitMatches: CounterfeitMatch[],
    primaryTests: Record<string, boolean>
  ): AssessmentLevel {
  ...
  private generateRecommendations(
    ...
    for (const violation of violations) {
      ...
      if (tenet) {
        recommendations.push(
          `Address ${tenet.name} violation: Review decision against "${tenet.decision_criteria[0]}"`
        );
      }
    }
    ...
    for (const match of counterfeitMatches) {
      recommendations.push(
        `Avoid counterfeit pattern "${match.counterfeit_pattern}". Consider authentic ${match.tenet_name} instead.`
      );
    }
    ...
    for (const tenet of lowScoreTenets.slice(0, 3)) {
      recommendations.push(
        `Strengthen ${tenet.name}: ${tenet.transformation_pattern}`
      );
    }
   ...
   */
  private isTenetRelevant(tenet: Tenet, textLower: string): boolean {
    // Foundation tenet (LOVE) is always relevant
    if (tenet.category === 'foundation') return true;
    ...
    if (textLower.includes(tenet.name.toLowerCase())) return true;
    if (textLower.includes(tenet.category)) return true;
   ...
   */
  private deepScore(tenet: Tenet, textLower: string, baseScore: number): number {
    let score = baseScore;
  ...
  quickEvaluate(decisionText: string): {
    ...
    let assessment: AssessmentLevel = 'caution';
