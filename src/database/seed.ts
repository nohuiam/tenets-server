...
import type { TenetCategory } from '../types.js';

const __filename = fileURLToPath(import.meta.url);
const __dirname = dirname(__filename);

interface TenetSeedData {
  id: number;
  name: string;
  definition: string;
...
interface SeedFile {
  tenets: TenetSeedData[];
}

/**
 * Load tenets from seed file and insert into database
 */
export function seedTenets(db: DatabaseManager): number {
 ...
 */
export function seedTenetsInline(db: DatabaseManager): number {
  const existingCount = db.getTenetCount();
  if (existingCount > 0) {
    return existingCount;
  }

  const tenets: TenetSeedData[] = [
    {
      id: 1,
      name: 'LOVE',
