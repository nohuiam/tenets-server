...
import type { SuggestRemediationOutput, Tenet } from '../types.js';
...
const RELATED_TENETS: Record<string, string[]> = {
...
export function createSuggestRemediationHandler(db: DatabaseManager) {
  return async (input: SuggestRemediationInput): Promise<SuggestRemediationOutput> => {
    ...
    remediationSteps.push(
      `Step ${remediationSteps.length + 1}: Directly address "${input.violation_description}" by reviewing against counterfeit patterns`
    );
